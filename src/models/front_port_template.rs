/*
 * NetBox API
 *
 * API to access NetBox
 *
 * The version of the OpenAPI document: 2.7
 * 
 * Generated by: https://openapi-generator.tech
 */




#[derive(Clone, Debug, PartialEq, Serialize, Deserialize)]
pub struct FrontPortTemplate {
    #[serde(rename = "id", skip_serializing_if = "Option::is_none")]
    pub id: Option<i32>,
    #[serde(rename = "device_type")]
    pub device_type: crate::models::NestedDeviceType,
    #[serde(rename = "name")]
    pub name: String,
    #[serde(rename = "type")]
    pub _type: crate::models::Status,
    #[serde(rename = "rear_port")]
    pub rear_port: crate::models::NestedRearPortTemplate,
    #[serde(rename = "rear_port_position", skip_serializing_if = "Option::is_none")]
    pub rear_port_position: Option<i32>,
}

impl FrontPortTemplate {
    pub fn new(device_type: crate::models::NestedDeviceType, name: String, _type: crate::models::Status, rear_port: crate::models::NestedRearPortTemplate) -> FrontPortTemplate {
        FrontPortTemplate {
            id: None,
            device_type,
            name,
            _type,
            rear_port,
            rear_port_position: None,
        }
    }
}


